...
double logisticFunction(std::vector<double>inputs, std::vector<double>weights, double bias) {
	double z = 0.0;
	z += bias;
...
double input_layer(std::vector<double> inputs, std::vector<double> weights, double bias) {
	double z = 0.0;
	for (size_t i = 0; i < inputs.size(); i++)
		z += inputs[i] * weights[i];
	return bias + z;
...
double delta_rule_for_learning_linear(double x, double y, double t, double e) {
...
*/
double delta_rule_for_learning_sigmoid(double x, double y, double t) {
	return -1 * x*y*(1 - y)*(t - y); //x = input value, y = actual output value, t = target output value
}

//y=1/1+e^-z
double SigmoidNeuron::sigmoid(double z) {
	return 1 / (1 + exp(-z));
}

double SigmoidNeuron::predict(std::vector<double>input) {
...
void SigmoidNeuron::train(std::vector<std::vector<double>> inputs, std::vector<int> labels, int array_dims) {
	double prediction = 0.0;
